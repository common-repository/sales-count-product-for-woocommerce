...
if ( ! class_exists( 'scpfw_kit' ) ) {
	class scpfw_kit {
		...
		function admin_menu() {
			add_submenu_page( 'sales-count-product-settings', esc_html__( 'Essential Kit', 'sales-count-product-for-woocommerce' ), esc_html__( 'Essential Kit', 'sales-count-product-for-woocommerce' ), 'manage_options', 'scpfw_kit', array(
				$this,
				'settings_page'
			) );
		}
		...
		function settings_page() {
            ...
            <div class="wrap">
                ...
                <div class="wp-list-table widefat plugin-install-network">
					...
					$updated      = false;
					$plugins_info = get_site_transient( 'scpfw_kit_plugins_info' );
											...
											<?php if ( $this->is_plugin_installed( $_plugin ) ) {
												if ( $this->is_plugin_active( $_plugin ) ) {
                                                    ...
                                                    <a href="<?php echo esc_url( $this->deactivate_plugin_link( $_plugin ) ); ?>"
                                                       class="button deactivate-now">
														<?php esc_html_e( 'Deactivate', 'sales-count-product-for-woocommerce' ); ?>
                                                    </a>
													<?php
												} else {
													?>
                                                    <a href="<?php echo esc_url( $this->activate_plugin_link( $_plugin ) ); ?>"
                                                       class="button activate-now">
														<?php esc_html_e( 'Activate', 'sales-count-product-for-woocommerce' ); ?>
                                                    </a>
													<?php
												}
											} else { ?>
                                                <a href="<?php echo esc_url( $this->install_plugin_link( $_plugin ) ); ?>"
                                                   class="install-now button wpckit-install-now">
													<?php esc_html_e( 'Install Now', 'sales-count-product-for-woocommerce' ); ?>
                                                </a>
											<?php } ?>
                                        </li>
                                        <li>
                                            <a href="<?php echo esc_url( $details_link ); ?>"
                                               ...
                                               aria-label="<?php echo esc_attr( sprintf( esc_html__( 'More information about %s', 'sales-count-product-for-woocommerce' ), $plugin_info['name'] ) ); ?>"
                                               data-title="<?php echo esc_attr( $plugin_info['name'] ); ?>">
												<?php esc_html_e( 'More Details', 'sales-count-product-for-woocommerce' ); ?>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
							...
							<?php 
								...
								if ( isset( $plugin_info['version'] ) ) { ?>
                                    <div class="column-updated">
										<?php echo esc_html__( 'Version', 'sales-count-product-for-woocommerce' ) . ' ' . $plugin_info['version']; ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['active_installs'] ) ) { ?>
                                    <div class="column-downloaded">
										<?php echo number_format_i18n( $plugin_info['active_installs'] ) . esc_html__( '+ Active Installations', 'sales-count-product-for-woocommerce' ); ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['last_updated'] ) ) { ?>
                                    <div class="column-compatibility">
                                        <strong><?php esc_html_e( 'Last Updated:', 'sales-count-product-for-woocommerce' ); ?></strong>
                                        <span><?php printf( esc_html__( '%s ago', 'sales-count-product-for-woocommerce' ), esc_html( human_time_diff( strtotime( $plugin_info['last_updated'] ) ) ) ); ?></span>
                                    </div>
								<?php }
					...
					if ( $updated ) {
						set_site_transient( 'scpfw_kit_plugins_info', $plugins_info, 24 * HOUR_IN_SECONDS );
					}
					?>
                </div>
            </div>
		...
		public function activate_plugin_link( $plugin) {
			...
			return wp_nonce_url( admin_url( 'admin.php?page=scpfw_kit&action=activate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'activate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
			
		}

		public function deactivate_plugin_link( $plugin ) {
			return wp_nonce_url( admin_url( 'admin.php?page=scpfw_kit&action=deactivate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'deactivate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
		}
	}
